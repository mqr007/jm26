...
var ServiceConfig = class _ServiceConfig {
  ...
  constructor(options) {
    ...
    this.name = import_lodash3.default.defaultTo(name, "jimeng-api");
  ...
  constructor() {
    ...
    this.app.use(async (ctx, next) => {
      if (!ctx._jsonProcessed) {
        await (0, import_koa_body.default)(Object.assign(import_lodash11.default.clone(config_default.system.requestBody), {
          multipart: true,
          // 开启multipart文件上传
          formidable: {
  ...
  while (retries <= maxRetries) {
    try {
      ...
      const response = await import_axios2.default.request({
        ...
        validateStatus: () => true,
        // 允许任何状态码
        ...import_lodash12.default.omit(options, "params", "headers")
      });
...
function checkResult(result) {
  ...
  if (!import_lodash12.default.isFinite(Number(ret))) return result.data;
...
var images_default = {
  ...
  post: {
    ...
    "/compositions": async (request2) => {
      ...
      const finalModel = import_lodash13.default.defaultTo(model, DEFAULT_IMAGE_MODEL);
    ...
    "/get_history_by_ids": async (request2) => {
      request2.validate("body.history_ids", (v) => import_lodash13.default.isArray(v) || import_lodash13.default.isString(v), 'history_ids \u5FC5\u987B\u662F\u6570\u7EC4\u683C\u5F0F\uFF08\u5982 ["id1", "id2"]\uFF09\u6216\u5B57\u7B26\u4E32\u683C\u5F0F\uFF08\u5982 "id1"\uFF09').validate("headers.authorization", import_lodash13.default.isString);
      const tokens = tokenSplit(request2.headers.authorization);
      const token = import_lodash13.default.sample(tokens);
      let { history_ids } = request2.body;
      if (import_lodash13.default.isString(history_ids)) {
      ...
      for (const id of history_ids) {
        if (!import_lodash13.default.isString(id)) {
          throw new Error("\u5386\u53F2\u8BB0\u5F55ID\u5FC5\u987B\u662F\u5B57\u7B26\u4E32\u7C7B\u578B");
        }
      }
      const result = await getImagesByHistoryIds(history_ids, token);
      const responseFormat = import_lodash13.default.defaultTo(request2.body.response_format, "url");
      let allImages = [];
  ...
  get: {
    "/history/:id": async (request2) => {
      request2.validate("params.id", import_lodash13.default.isString).validate("headers.authorization", import_lodash13.default.isString);
      const tokens = tokenSplit(request2.headers.authorization);
      const token = import_lodash13.default.sample(tokens);
...
async function uploadImageBuffer(imageBuffer, refreshToken, regionInfo) {
  ...
  try {
    ...
    const payloadHash = import_crypto4.default.createHash("sha256").update(commitPayload, "utf8").digest("hex");
    const commitRequestHeaders = {
      "x-amz-date": commitTimestamp,
      "x-amz-security-token": session_token,
      "x-amz-content-sha256": payloadHash
...
async function generateVideo(_model, prompt, {
  ...
  let uploadIDs = [];
  const uploadedFiles = import_lodash14.default.values(files);
...
var chat_default = {
  ...
  post: {
    "/completions": async (request2) => {
      request2.validate("body.model", (v) => import_lodash15.default.isUndefined(v) || import_lodash15.default.isString(v)).validate("body.messages", import_lodash15.default.isArray).validate("headers.authorization", import_lodash15.default.isString);
      const tokens = tokenSplit(request2.headers.authorization);
      const token = import_lodash15.default.sample(tokens);
...
var token_default = {
  ...
  post: {
    "/check": async (request2) => {
      request2.validate("body.token", import_lodash16.default.isString);
    ...
    "/points": async (request2) => {
      request2.validate("headers.authorization", import_lodash16.default.isString);
      const tokens = tokenSplit(request2.headers.authorization);
...
var videos_default = {
  ...
  post: {
    "/generations": async (request2) => {
      const contentType = request2.headers["content-type"] || "";
      const isMultiPart = contentType.startsWith("multipart/form-data");
      request2.validate("body.model", (v) => import_lodash17.default.isUndefined(v) || import_lodash17.default.isString(v)).validate("body.prompt", import_lodash17.default.isString).validate("body.ratio", (v) => import_lodash17.default.isUndefined(v) || import_lodash17.default.isString(v)).validate("body.resolution", (v) => import_lodash17.default.isUndefined(v) || import_lodash17.default.isString(v)).validate("body.duration", (v) => {
        if (import_lodash17.default.isUndefined(v)) return true;
        if (isMultiPart && typeof v === "string") {
          const num = parseInt(v);
          return num === 5 || num === 10;
        }
        return import_lodash17.default.isFinite(v) && (v === 5 || v === 10);
      }).validate("body.file_paths", (v) => import_lodash17.default.isUndefined(v) || import_lodash17.default.isArray(v)).validate("body.filePaths", (v) => import_lodash17.default.isUndefined(v) || import_lodash17.default.isArray(v)).validate("body.response_format", (v) => import_lodash17.default.isUndefined(v) || import_lodash17.default.isString(v)).validate("headers.authorization", import_lodash17.default.isString);
      const tokens = tokenSplit(request2.headers.authorization);
      const token = import_lodash17.default.sample(tokens);
      const {
